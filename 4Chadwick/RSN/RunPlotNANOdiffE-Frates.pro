...
; February 7th, 2020
;
; Display the 3-month (12-week) and 6-month (24-week) Rates.
;
  P = PLOT( NANO1DAY_TIME, RATE12WK,  LINESTYLE='NONE', BUFFER=1,  $
            SYMBOL='o', SYM_COLOR='PURPLE', SYM_FILLED=1,  $
            TITLE='OOI-BPR Difference (MJ03E - MJ03F) Long-Term Average Rates of Depth Change.', $
           XTICKFORMAT='(C(CDI,1x,CMoA,1X,CYI))',  XMINOR=11, XSTYLE=1,     $
           XTITLE=T, YTITLE='cm/year', YRANGE=[MN,MX], DIMENSION=[1024,512] )
  S = SYMBOL( /NORMAL, 0.40, 0.8, 'o',  LABEL_COLOR='PURPLE',  LABEL_STRING='3-month',  $
              /SYM_FILLED, SYM_COLOR='PURPLE'  )
  S = PLOT( NANO1DAY_TIME, RATE24WK,  LINESTYLE='NONE',   $
           /OVERPLOT, YRANGE=[MN,MX], $  ; So the Y-Axis will not be adjusted.
            SYMBOL='o', SYM_COLOR='ORANGE', SYM_FILLED=1   )
  S = SYMBOL( /NORMAL, 0.60, 0.8, 'o',  LABEL_COLOR='ORANGE', LABEL_STRING='6-month', $
              /SYM_FILLED, SYM_COLOR='ORANGE' )
;
  WRITE_PNG, '~/4Chadwick/RSN/MJ03F/NANOdifferenceLTrates2E-F.png', P.CopyWindow()
  PRINT, SYSTIME() + ' MJ03F/NANOdifferenceLTrates2E-F.png is created.'
;
;
...
; February 7th, 2020
;
; Display the same info as above except for 6 months long data.
;
; Use the matched times: MJ03JF_TIME[JF] and locate the date 3 months from the current data.
;
  N = N_ELEMENTS( NANO_TIME )     - 1
  ...
  IF NANO_TIME[N] LT NANO1DAY_TIME[M] THEN T = NANO_TIME[N] ELSE T = NANO1DAY_TIME[M]
  N = LOCATE_TIME_POSITION( NANO_TIME, T )
  T = ROUND( T -  182 ) - 0.5  ; Move back 182.5 days or 6 months.
; The ROUND( ) - 0.5 will make sure the date starts at 00:00:00.
  PRINT, FORMAT='(C())', T  ; for checking.
  I = LOCATE_TIME_POSITION( NANO_TIME, T )
;
...
;
  P = PLOT( NANO_TIME[I:N], NANO_DIFF[I:N], 'r', BUFFER=1, DIMENSION=[1024,512],  $
            TITLE='OOI-BPR Difference: MJ03E - MJ03F, last 6 months',             $
           XTICKFORMAT='(C(CDI,1x,CMoA,1X,CYI))',  XSTYLE=1, XMINOR=29, YRANGE=Y, $
           XTITLE=T,  YTITLE='De-tided Depth Difference in meters' )
;
  CALDAT, NANO_TIME[I],  M,D,YR  ; Get M)onth, D)ay & Y)ear.
...
; Convert the Rate of Depth (meters) Change/Day into cm/ATE =  RATE*100.0*
  ...
  RATE = STRING( FORMAT='(F5.1)', RATE )  ; into a string: '19.1" e.g.
;
  T = TEXT( COLOR='ORANGE',  /NORMAL, 0.15, 0.03,           $
            '6-month avg. uplift rate: ' + RATE + ' cm/yr'  )
;
  P = PLOT(  /OVERPLOT, NANO1DAY_TIME[S:M], Y, COLOR='ORANGE', THICK=2 )
; P.ORDER, /BRING_TO_FRONT  ; at Caldera P.C. only.
;
  T = ROUND( NANO1DAY_TIME[M] - 84 )  ; Move back 84 days or 3 months.
  S = LOCATE_TIME_POSITION( NANO1DAY_TIME, T )
;
  PRINT, FORMAT='(C())', NANO1DAY_TIME[S], T
;
...
; Convert the Rate of Depth (meters) Change/Day into cm/year.
  ...
  RATE = STRING( FORMAT='(F5.1)', RATE )  ; into a string: '19.1" e.g.
;
  T = TEXT( COLOR='PURPLE', /NORMAL, 0.65, 0.03,            $
            '3-month avg. uplift rate: ' + RATE + ' cm/yr'  )
;
  P = PLOT(  /OVERPLOT, NANO1DAY_TIME[S:M], Y, COLOR='PURPLE', THICK=2 )
; P.ORDER, /BRING_TO_FRONT  ; at Caldera P.C. only.
;
...
; Label the dot symbols.
;
  S = SYMBOL( /NORMAL, 0.120, 0.07, 'o', /SYM_FILLED, SYM_COLOR='ORANGE' )
  S = SYMBOL( /NORMAL, 0.135, 0.07, 'o', /SYM_FILLED, SYM_COLOR='PURPLE',$
               LABEL_STRING='1-Day Means' )
;
;
  WRITE_PNG, '~/4Chadwick/RSN/MJ03F/NANOratesLast6monthsE-F.png', P.CopyWindow()  ; IDL 8.5 or hight.
; P.SAVE,    '~/4Chadwick/RSN/MJ03F/NANOratesLast6monthsE-F.png', WIDTH=2048, HIGTH=1024
  PRINT, SYSTIME() + ' MJ03F/NANOratesLast6monthsE-F.png  is created.'
;
