...
;
  IDL_FILE = OUTPUT_DIRECTORY + FILE_ORIG + PATH_SEP()  $
                              + FILE_ORIG + '-NANO.idl'
;                      '3Day' + FILE_ORIG + '-NANO.idl' ; After March 2015
;
...
;
  PRINT, 'Detection Parameter will be computed at ',  $
...
; Note that only part oh the 2 NANO arrays will be sent such as
...
;
   PRINT, 'The Offset Index: ' + STRTRIM( I, 2 )   $
        + ' >= ' + STRTRIM( M, 2 ) + '!'
   PRINT, 'Not Enough Data Points.  Detection Parameter will not be computed!'
;
...
;  PLOT_NANO_DATA,       IDL_FILE, [-3,0], SHOW_PLOT=0,UPDATE_PLOTS=1  ; After March 2015
ENDIF
;
RETURN
