...
;.RUN ~/idl/match.pro
;
  PRINT, SYSTIME() + ' Start running of RunPlotCTD4MJ03F.pro ...'
;
...
;
  RESTORE, '~/4Chadwick/RSN/MJ03F/CTD-MJ03F.idl'
; RESTORE, '/data/lau/4Chadwick/RSN/MJ03F/CTD-MJ03F.idl'
;
...
; P = PLOT( CTD_TIME   , CTD_TEMP   , 'r', BUFFER=B, DIMENSION=[800, 256],  $
  P = PLOT( CTD_TIME[I], CTD_TEMP[I], 'r', BUFFER=B, DIMENSION=[800, 256],  $
            TITLE='MJ03F CTD Temperature data, Start to Present.',          $
           XTICKFORMAT='(C(CDI,1X,CMoA,1X,CYI))',     XMINOR=11,            $
           XTITLE=T, YTITLE='$Temperature in ^{o}C$', XSTYLE=1 )
;
  WRITE_PNG, '~/4Chadwick/RSN/MJ03F/MJ03FCTDTempAll.png', P.CopyWindow()
  PRINT, SYSTIME() + ' MJ03F/MJ03FCTDTempAll.png is created.'
;
...
; P = PLOT( CTD_TIME   , DENSITY   , 'b', BUFFER=B, DIMENSION=[800, 256],     $
  P = PLOT( CTD_TIME[I], DENSITY[I], 'b', BUFFER=B, DIMENSION=[800, 256],     $
            TITLE='MJ03F CTD Density data, Start to Present.',                $
           XTICKFORMAT='(C(CDI,1X,CMoA,1X,CYI))',  XMINOR=11, YRANGE=[MN,MX], $
           XTITLE=T, YTITLE='$Density: kg/m^{3}$', XSTYLE=1 )
;
  WRITE_PNG, '~/4Chadwick/RSN/MJ03F/MJ03FCTDDensAll.png', P.CopyWindow()
  PRINT, SYSTIME() + ' MJ03F/MJ03FCTDDensAll.png is created.'
;
...
; P = PLOT( CTD_TIME   , SALINITY  , 'g', BUFFER=B, DIMENSION=[800, 256],     $
  P = PLOT( CTD_TIME[I], SALINITY[I],'g', BUFFER=B, DIMENSION=[800, 256],     $
            TITLE='MJ03F CTD Salinity data, Start to Present.',               $
           XTICKFORMAT='(C(CDI,1X,CMoA,1X,CYI))', XMINOR=11, YRANGE=[MN,MX],  $
           XTITLE=T, YTITLE='Salinity: ppt',      XSTYLE=1 )
;
  WRITE_PNG, '~/4Chadwick/RSN/MJ03F/MJ03FCTDSalAll.png', P.CopyWindow()
  PRINT, SYSTIME() + ' MJ03F/MJ03FCTDSalAll.png is created.'
;
; Save the P.XTICKVALUES used for plotting all the data.
; in case total data less than 6 months.
;
  MX = P.XTICKVALUES  ; August 31st, 2020
;
...
;
; Reset the M above if the total data are less than 6 months.
; Use the P.XTICKVALUES used for all the data instead.
;
  IF CTD_TIME[0] GT T THEN M = MX  ;  August 31st, 2020
;
; Create a Time Range for labelling.
;
...
; P = PLOT( CTD_TIME[K:N], CTD_TEMP[K:N], 'r',    DIMENSION=[800, 256],   $
  P = PLOT( CTD_TIME[ I ], CTD_TEMP[ I ], 'r',    DIMENSION=[800, 256],   $
            TITLE='MJ03F CTD Temperature data, last 6 months', BUFFER=B,  $
           XTICKFORMAT='(C(CDI,1X,CMoA,1X,CYI))',     XTICKVALUES=M,      $
           XTITLE=T, YTITLE='$Temperature in ^{o}C$', XSTYLE=1, XMINOR=29 )
;
  WRITE_PNG, '~/4Chadwick/RSN/MJ03F/MJ03FCTDTemp6mo.png', P.CopyWindow()
  PRINT, SYSTIME() + ' MJ03F/MJ03FCTDTemp6mo.png is created.'
;
...
; P = PLOT( CTD_TIME[K:N],     DENSITY[K:N], 'b', DIMENSION=[800, 256],        $
  P = PLOT( CTD_TIME[ I ],     DENSITY[ I ], 'b', DIMENSION=[800, 256],        $
            TITLE='MJ03F CTD Density data, last 6 months', BUFFER=B,           $
           XTICKFORMAT='(C(CDI,1X,CMoA,1X,CYI))',  XMINOR=29, YRANGE=[MN,MX],  $
           XTITLE=T, YTITLE='$Density: kg/m^{3}$', XTICKVALUES=M, XSTYLE=1     )
;
  WRITE_PNG, '~/4Chadwick/RSN/MJ03F/MJ03FCTDDens6mo.png', P.CopyWindow()
  PRINT, SYSTIME() + ' MJ03F/MJ03FCTDDens6mo.png is created.'
;
...
; P = PLOT( CTD_TIME[K:N],     SALINITY[K:N],'g', DIMENSION=[800, 256],       $
  P = PLOT( CTD_TIME[ I ],     SALINITY[ I ],'g', DIMENSION=[800, 256],       $
            TITLE='MJ03F CTD Salinity data, last 6 months', BUFFER=B,         $
           XTICKFORMAT='(C(CDI,1X,CMoA,1X,CYI))', XMINOR=29, YRANGE=[MN,MX],  $
           XTITLE=T, YTITLE='Salinity: ppt', XTICKVALUES=M,  XSTYLE=1          )
;
  WRITE_PNG, '~/4Chadwick/RSN/MJ03F/MJ03FCTDSal6mo.png', P.CopyWindow()
  PRINT, SYSTIME() + ' MJ03F/MJ03FCTDSal6mo.png is created.'
;
;
...
;           TITLE='OOI-BPR Difference: MJ03F - MJ03F, last 7 Days.',              $
...
;
  PRINT, SYSTIME() + ' Finish running of RunPlotCTD4MJ03F.pro.'
;
; End of File: RunPlotCTD4MJ03F.pro
