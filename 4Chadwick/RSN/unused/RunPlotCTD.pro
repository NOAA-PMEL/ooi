...
;.RUN ~/idl/match.pro
;
  PRINT, SYSTIME() + ' Start running of RunPlotCTD.pro ...'
;
...
;
  S  = STDEV( DENSITY,  M )  ; Get the Standard Deviation (S) and the average (M).
  P  = S*3
...
;
  S  = STDEV( SALINITY, M )  ; Get the Standard Deviation (S) and the average (M).
  P  = S*3
  MX = M + P  ; Upper  Plotting
  MN = M - P  ; Lower  Range for Y-Axis.
;
...
;
  PRINT, SYSTIME() + ' Finish running of RunPlotCTD.pro.'
;
; End of File: RunPlotCTD.pro
