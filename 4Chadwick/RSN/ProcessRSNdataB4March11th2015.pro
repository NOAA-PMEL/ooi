...
;  Calculate the Tilt Magnitudes & Directions from the LILY data.
   ...
   IDL_FILE += FILE_ORIG + '-LILY.idl'           ; Before March 2015
;  IDL_FILE will be = '/RSN/Output/MJ03F/MJ03F-LILY.idl' for example.
;  IDL_FILE += '3Day' + FILE_ORIG + '-LILY.idl'  ; After March 2015
...
;        PLOT_LILY_DATA, IDL_FILE,[-3,0], SHOW_PLOT=0,UPDATE_PLOTS=1  ; 1=Yes  After March 2015
      ENDIF  ; for plotting.
...
IF N_IRIS_CNT GT 0 THEN  BEGIN
   IDL_FILE  = OUTPUT_DIRECTORY + FILE_ORIG + PATH_SEP()
   FILE_ID   = IDL_FILE  ; Get the same Output Directory Path.
   IDL_FILE +=          FILE_ORIG + '-IRIS.idl'  ; Before March 2015
;  IDL_FILE += '3Day' + FILE_ORIG + '-IRIS.idl'  ; After  March 2015
...
;        PLOT_IRIS_DATA, IDL_FILE,[-3,0], SHOW_PLOT=0,UPDATE_PLOTS=1  ; 0=No  After March 2015
...
IF N_HEAT_CNT GT 0 THEN  BEGIN
   IDL_FILE  = OUTPUT_DIRECTORY + FILE_ORIG + PATH_SEP()
   FILE_ID   = IDL_FILE  ; Get the same Output Directory Path.
   IDL_FILE +=          FILE_ORIG + '-HEAT.idl'  ; Before March 2015
;  IDL_FILE += '3Day' + FILE_ORIG + '-HEAT.idl'  ; After  March 2015
...
;        PLOT_HEAT_DATA, IDL_FILE,[-3,0], SHOW_PLOT=0,UPDATE_PLOTS=1  ; 1=Yes  After March 2015
      ENDIF  ; for plotting.
   ENDIF  ; Checking.
ENDIF
IF N_NANO_CNT GT 0 THEN  BEGIN
   IDL_FILE  = OUTPUT_DIRECTORY + FILE_ORIG + PATH_SEP()
   FILE_ID   = IDL_FILE  ; Get the same Output Directory Path.
   IDL_FILE +=          FILE_ORIG + '-NANO.idl'  ; Before March 2015
;  IDL_FILE += '3Day' + FILE_ORIG + '-NANO.idl'  ; After  March 2015
...
; Revised: February 9th, 2015
;
PRO PROCESS_RSN_DATA_FILE,  RSN_FILE,  $ ;  Input File Name.
            STATUS,  $ ; Output: 'Nono' or 'MJ03D File processed' e.g.
            OUTPUT2=OUTPUT_DIRECTORY     ; Input : Directory name.
;
...
;        Note that the procedure below in the file: StatusFile4RSN.pro
      ...
      IF STATUS NE 'None' THEN  BEGIN  ; for checking alert events & plotting.
